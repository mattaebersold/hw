  ...
  return (
    <nav className="bg-gray-900 border-b border-gray-800 sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <Link to="/" className="font-bold text-white tracking-tight flex items-center gap-2 mt-1">
          <span className="text-red-500 text-3xl relative -mt-3">🏎</span> HotWheels Market
        </Link>
            ...
            <>
              <Link to={`/profile/${user._id}`} className="flex items-center gap-2 group">
                <img
                  ...
                  alt={user.username}
                  className="w-9 aspect-square rounded-full object-cover transition-colors"
                />
              </Link>
              <Link
                to="/create"
                className="w-9 aspect-square bg-red-600 hover:bg-red-500 text-white rounded-full flex items-center justify-center text-3xl font-bold leading-none transition-colors"
                title="List a car"
              >
